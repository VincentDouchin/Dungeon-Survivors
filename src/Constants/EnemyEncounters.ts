...
const ENENMYENCOUNTERS: Record<enemyWaveName, () => Encounter> = {
	ORCS: () => new Encounter() // 23 waves 605 enemies
		...
		.waitForEnemiesCleared()
		.addWave([[orcBig, 1], [orcBigPatch, 1]], 1, 0)
		.addWave([[goblin, 20], [orcMasked, 8], [orcShaman, 5], [orc, 10]], 1) // 44
		.waitForEnemiesCleared()
		.stop(),
	ANIMALS: () => new Encounter() // 21 Waves 498 enemies
		...
		.waitForEnemiesCleared()
		.addWave([[mushroomBig, 1]], 1, 0)
		.addWave([[mushroomSmall, 30], [mushroomMedium, 10], [berserk(mushroomMedium), 5]], 1) // 46
		.waitForEnemiesCleared()
		.stop(),
	ELVES: () => new Encounter() // 18 waves 512 enemies
		...
		.waitForEnemiesCleared()
		.addWave([[leprechaun, 10], [berserk(woodElfMale), 3], [woodElfMale, 5], [woodElfFemale, 5], [berserk(woodElfFemale), 3], [elfRanger, 3]], 4) // 120
		.waitForEnemiesCleared()
	...
	VILLAGERS: () => new Encounter() // 21 waves 636 enemies
		...
		.waitForEnemiesCleared()
		.addWave([[blacksmith, 3], [butcher, 5], [berserk(butcher), 5], [villagerFemale, 10], [villagerMale, 10]], 4) // 150
		.waitForEnemiesCleared()
		.addWave([[blacksmith, 5], [butcher, 5], [villagerFemale, 15], [villagerMale, 15]], 3) // 225
		.waitForEnemiesCleared()
		.addWave([[executioner, 1]], 1, 0)
		.addWave([[villagerFemale, 10], [villagerMale, 10], [blacksmith, 5], [butcher, 5]], 1) // 16
		.waitForEnemiesCleared()
		.stop(),
	PRIESTS: () => new Encounter()
		.addWave([[clericFat, 5], [clericNormal, 5], [clericSkinny, 5], [berserk(clericFat), 2], [berserk(clericNormal), 2], [berserk(clericSkinny), 2]], 5)
		.waitForEnemiesCleared()
	...
	UNDEAD: () => new Encounter()
		...
		.waitForEnemiesCleared()
		.addWave([[muddy, 2], [swampy, 2], [skeleton, 5], [zombieSmall, 10]], 5)
		.waitForEnemiesCleared()
		.addWave([[iceZombie, 3], [skeleton, 10], [muddy, 2], [swampy, 2]], 4)
		.waitForEnemiesCleared()
		.addWave([[skeleton, 15], [zombie, 5], [zombieSmall, 10]], 3)
		.waitForEnemiesCleared()
		.addWave([[ogre, 1]], 1, 0)
		.addWave([[iceZombie, 5], [skeleton, 10], [zombie, 10], [zombieSmall, 20]], 1)
		.waitForEnemiesCleared()
		.stop(),
	...
	VAMPIRES: () => new Encounter()
		.addWave([[cultist, 7], [bat, 7], [berserk(bat), 3], [berserk(cultist), 3]], 5)
		.waitForEnemiesCleared()
