...
class TouchController extends InputController {
	...
	constructor(inputs: INPUTS[]) {
		...
		ECS.eventBus.subscribe(UIEVENTS.TOUCH, ({ input, amount, entity }) => {
			...
			const unSubResetOnUpTouch = ECS.eventBus.subscribe('up', ({ uiObjects, objects }) => {
			...
			subs.push(unSubResetOnUpTouch)
			const resetOnDestroy = ECS.eventBus.subscribe(ECSEVENTS.DELETE_ENTITY, (unknownEntity) => {
				if (entity === unknownEntity) {
					this.inputs.set(input, 0)
					subs.forEach(sub => sub())
				}
			})
			subs.push(resetOnDestroy)
		})
	}
